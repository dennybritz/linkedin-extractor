...
ProfileParser.prototype.getEducation = function() {
  ...
  return items.map(function(i, element){
    ...
    var schoolName = $div.find('header h4.summary').text();
    var degree = $div.find('.degree').text().match(/(.*?)(?:,\s)?$/)[1];
    var fieldOfStudy = $div.find('.major').text();
    ...
    var activities = $div.find('.activities').text();
    var notes = $div.find('.notes').text();
